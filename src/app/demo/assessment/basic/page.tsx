...
export default function DemoBasicAssessmentPage() {
  const router = useRouter()
  const children = DEMO_CHILDREN.map((c) => ({ id: c.id, name: c.name, grade: c.grade }))
  ...
  const goForward = () => { setDirection('forward'); setCurrentStep((s) => s + 1) }
  const goBack = () => { setDirection('backward'); setCurrentStep((s) => s - 1) }
  ...
  const handleFinish = () => {
    ...
    setTimeout(() => router.push('/demo/report'), 1500)
  }
          ...
          <div className="w-full max-w-md animate-fade-in">
            ...
            <div className="bg-white rounded-2xl shadow-md p-6 space-y-4">
              ...
              <div>
                ...
                <input
                  type="text" value={grade} onChange={(e) => setGrade(e.target.value)} placeholder="小4"
                  className="w-full border border-gray-300 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-[#F7941D] focus:border-transparent text-lg"
                />
              </div>
            </div>
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] flex flex-col">
      ...
      <div className="h-1 bg-gray-200">
        <div className="h-1 bg-[#F7941D] transition-all duration-500 ease-out" style={{ width: `${progress}%` }} />
      </div>
        ...
        <div key={currentStep} className={`w-full max-w-md ${animClass}`}>
          ...
          {q.type === 'radio' && (
            <div className="space-y-3">
              {q.options.map((opt) => (
                <button key={opt.value} onClick={() => handleRadioAnswer(q.key, opt.value)}
            ...
            <>
              <div className="grid grid-cols-2 gap-3">
                {q.options.map((skill) => {
                  ...
                  return (
                    <button key={skill}
                      onClick={() => setQ7Skills((prev) => prev.includes(skill) ? prev.filter((x) => x !== skill) : [...prev, skill])}
                      className={`py-4 px-4 text-lg rounded-xl border-2 transition active:scale-[0.98] flex items-center justify-center gap-2 ${
                        isChecked ? 'border-[#F7941D] bg-[#FFF8F0] text-[#1B2A4A] font-medium' : 'border-gray-200 bg-white text-gray-700 hover:border-[#F7941D]/50'
                      }`}
                    >
                      <span className={`w-5 h-5 rounded flex-shrink-0 border-2 flex items-center justify-center ${isChecked ? 'border-[#F7941D] bg-[#F7941D]' : 'border-gray-300'}`}>
                        {isChecked && <svg className="w-3 h-3 text-white" viewBox="0 0 12 12" fill="none"><path d="M2 6l3 3 5-5" stroke="currentColor" strokeWidth="2" /></svg>}
                      </span>
                      {skill}
                    </button>
                  )
              ...
              <button onClick={goForward} disabled={q7Skills.length === 0}
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      {SUBJECTS.map((s) => {
                        ...
                        return (
                          <button key={`good-${s}`}
                            onClick={() => setQ17Good((prev) => prev.includes(s) ? prev.filter((x) => x !== s) : [...prev, s])}
                            className={`w-full py-2 px-3 text-left rounded-lg border transition flex items-center gap-2 ${
                              isChecked ? 'border-[#43A047] bg-green-50 text-[#43A047] font-medium' : 'border-gray-200 text-gray-700 hover:border-[#43A047]/50'
                            }`}
                          >
                            <span className={`w-4 h-4 rounded flex-shrink-0 border-2 flex items-center justify-center ${isChecked ? 'border-[#43A047] bg-[#43A047]' : 'border-gray-300'}`}>
                              {isChecked && <svg className="w-2.5 h-2.5 text-white" viewBox="0 0 12 12" fill="none"><path d="M2 6l3 3 5-5" stroke="currentColor" strokeWidth="2" /></svg>}
                            </span>
                            {s}
                          </button>
                        )
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      {SUBJECTS.map((s) => {
                        ...
                        return (
                          <button key={`weak-${s}`}
                            onClick={() => setQ17Weak((prev) => prev.includes(s) ? prev.filter((x) => x !== s) : [...prev, s])}
                            className={`w-full py-2 px-3 text-left rounded-lg border transition flex items-center gap-2 ${
                              isChecked ? 'border-red-500 bg-red-50 text-red-500 font-medium' : 'border-gray-200 text-gray-700 hover:border-red-300'
                            }`}
                          >
                            <span className={`w-4 h-4 rounded flex-shrink-0 border-2 flex items-center justify-center ${isChecked ? 'border-red-500 bg-red-500' : 'border-gray-300'}`}>
                              {isChecked && <svg className="w-2.5 h-2.5 text-white" viewBox="0 0 12 12" fill="none"><path d="M2 6l3 3 5-5" stroke="currentColor" strokeWidth="2" /></svg>}
                            </span>
                            {s}
                          </button>
          ...
          >← {currentStep === 1 ? '基本情報に戻る' : '前の質問に戻る'}</button>
        </div>
      </div>
    </div>
  )
